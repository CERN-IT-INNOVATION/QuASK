...
class Ansatz:
    ...
    def change_wires(self, operation_index: int, new_wires: List[int]):
        ...
        assert 0 <= new_wires[0] < self.n_qubits, "First wire index out of bounds"
        assert 0 <= new_wires[1] < self.n_qubits, "Second wire index out of bounds"
        assert new_wires[0] != new_wires[1], "Cannot specify the same wire twice"
    ...
    def from_numpy(array, n_features, n_qubits, n_operations, allow_midcircuit_measurement):
        ...
        for i in range(n_operations):
            ans.operation_list[i] = Operation.from_numpy(array[i*5:(i+1)*5])
        return ans
